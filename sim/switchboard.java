...
// $Id: switchboard.java,v 1.28 2012/02/11 23:22:36 drmiller Exp $
...
{
	final String ident = "$Id: switchboard.java,v 1.28 2012/02/11 23:22:36 drmiller Exp $";
		...
		implements Runnable, KeyListener
{
	...
	private class Kellogg_Cabinet_Overlay extends JPanel
	{
		...
		public void paint(Graphics g) {
			...
			if (_sel_plug != null) {
				...
				g2d.fillRect(l.x - 7, l.y - 100, 14, 36);
				g2d.fillRect(l.x - 7, l.y - 106, 14, 3);
				...
				g2d.drawLine(l.x - 5, l.y - 99, l.x - 5, l.y - 65);
	...
	public Kellogg_Cabinet(int num_lines, int num_circs,
		...
		_scroll.setPreferredSize(new Dimension(
			cab_width - Kellogg_Magneto.obj_width,
		...
		JPanel ro = makeRODropCircPanel(circs, num_circs, top, font_metrics);
		s.gridx = 0;
		s.gridy = 1;
		s.gridwidth = 1;
		s.gridheight = 1;
	...
	private JPanel makeRODropCircPanel(JPanel circs, int num_circs,
		...
		for (x = 0; x < num_circs; ++x) {
			Kellogg_Drop drp1 = new Kellogg_Drop(x + 1,
						Kellogg_Circuit.obj_width, font_metrics);
			s.gridx = x;
			s.gridy = 0;
			s.gridwidth = 1;
			s.gridheight = 1;
	...
	implements MouseListener, KeyListener
{
	...
	public Kellogg_RingSw(Component top) {
		_state = OFF;
		setPreferredSize(new Dimension(obj_width, obj_height));
		setOpaque(false);
		setForeground(Color.black);
...
class Kellogg_Circuit extends JPanel
{
	...
	public Kellogg_Circuit(JPanel parent, int num, Kellogg_Cabinet cab,
			...
			Component top, FontMetrics font_metrics) {
		...
		_ring = new Kellogg_RingSw(top);
		_call = new Kellogg_Plug(this, cab, _drop_call, Kellogg_RingSw.PULL);
		_ans = new Kellogg_Plug(this, cab, _drop_ans, Kellogg_RingSw.PUSH);
